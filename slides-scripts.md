...
→ 那我们团队具体用什么工具跑这套 Harness?先看我们的工程环境。

---

## S9 — 落地装备:工具 × Skill(蓝区:我们的工程环境)

**视觉**:左右分栏——左「工具层」三个工具竖排 + 双向箭头标注 | 右「Skill 层」三行表格

**口播要点**
- 在进入 7 阶段流程之前,先交代我们团队的工程环境——用什么工具、靠什么 Skill 把这套 Harness 跑起来

**工具层(左)**

| 工具 | 角色 |
|---|---|
| Claude Code | 主力 agent,日常交互、写代码、跑 skill |
| Codex ↔ Claude Code | 主力 agent,互相交叉检视(Writer/Reviewer 模式的工具层落地) |
| OpenCode (GLM5) | 低成本长任务委托,通过 MCP opencode-bridge 异步执行 |

**Skill 层(右)**

| Skill | 一句话定位 | 主要触发时机 |
|---|---|---|
| Grill-me | 让 agent 反向盘问你,把隐藏假设逼出来 | 进入 Design 前 |
| OpenSpec | 规格状态机,把需求变成 agent 的工作合同 | Design → Implement |
| Superpowers | 完整开发工作流插件,把 spec-first、TDD、subagent 等最佳实践打包成自动触发的技能集 | 全程 |

→ 工具和 Skill 就位,下面看这套流程的完整框架。

---

## S10 ★ 7 阶段工作流框架(承上启下核心页)
...
### S11 — 阶段 0:Bootstrap:先让 agent 读懂项目
...
### S17 — 阶段 4:Verify 是什么 + 为什么需要 eval-loop
...
### S22 — 阶段 6:Retrospective — 整个 loop 的复利引擎
...
### S23 — 反馈回路:Harness 优化 Harness

**视觉**:回到 S10 的螺旋图,这次重点高亮 Retrospective → Bootstrap / Design 这条回流箭头
